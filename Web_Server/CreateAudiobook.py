...
def split_text_into_sentences(text: str, max_length_chunk: int = DEFAULT_MAX_CHUNK_LENGTH) -> list:
    ...
    try:
        ...
        avg_length_chunk = int(avg_length_sentences) / 10
        avg_length_chunk = int(avg_length_chunk) 
        print("The average chunk length before extending to default value is : " + str(avg_length_chunk) )
        if avg_length_chunk < DEFAULT_MAX_CHUNK_LENGTH : 
            print("Extending the average chunk length to fit the default value for the chunk length but ensure also that the chunklength represents the average sentence length to improve the speech-melody. ")
            while avg_length_chunk < DEFAULT_MAX_CHUNK_LENGTH :  
                avg_length_chunk += avg_length_chunk 
            print(f"The average chunk length is {avg_length_chunk} characters.")
            #max_length_chunk = min(avg_length_chunk, max_length_sentences)
            max_length_chunk = avg_length_chunk 
        else : 
            max_length_chunk = min(DEFAULT_MAX_CHUNK_LENGTH, max_length_sentences)
...
def remove_tabs_from_sentences(sentences):
    cleaned_sentences = []
    for sentence in sentences:
        if not sentence.strip():  # Skip empty or whitespace-only sentences
            print(f"Skipping empty or whitespace-only sentence: '{sentence}'")
            continue
        try:
